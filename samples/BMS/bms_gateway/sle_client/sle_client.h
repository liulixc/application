 ...
 */
  ...
  */
 typedef enum {
     NODE_ROLE_ORPHAN = 0, // 孤儿节点，尚未加入网络
     NODE_ROLE_MEMBER = 1, // 成员节点，已加入网络
  ...
  * @brief "加入网络"命令结构定义
  */
 #define ADOPTION_CMD 0x01 // 自定义加入网络命令的操作码
 typedef struct {
  ...
  */
  typedef struct {
    uint8_t origin_mac[6]; // 原始数据发送节点的MAC地址
    char    data[];        // 可变长数据 (通常为JSON格式字符串)
 ...
 typedef struct {
     ...
     uint16_t write_handle;  // 向该子节点传输数据的句柄
 } child_node_info_t;
