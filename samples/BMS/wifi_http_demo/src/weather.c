...
void parse_weather_data(const char *json_data)
{
    ...
    if (cityId)
    {
        ...
        if (end)
        {
            char cityId_value[50];
            strncpy(cityId_value, cityId, end - cityId);
            cityId_value[end - cityId] = '\0'; // 添加字符串结束符
            const char *city_name = get_city_name(cityId_value);
            osal_printk("City: %s\r\n", city_name);
        }
    }
    ...
    if (sd)
    {
        ...
        {
            char sd_value[10];
            strncpy(sd_value, sd, end - sd);
            sd_value[end - sd] = '\0'; // 添加字符串结束符
    ...
    if (qw)
    {
        ...
        {
            char qw_value[10];
            strncpy(qw_value, qw, end - qw);
            qw_value[end - qw] = '\0'; // 添加字符串结束符
            osal_printk("Temperature: %s°C\r\n", qw_value);
        }
    }
