/*
 * @file ble_server.c
 * @brief BLE（蓝牙低功耗）服务端示例代码，实现了BLE服务的注册、特征添加、回调注册、数据收发等功能。
 *
 * 主要功能：
 * 1. 初始化BLE服务端，注册GATT服务、特征、描述符。
 * 2. 处理BLE连接、断开、MTU变化等事件。
 * 3. 通过串口接收数据并通过BLE通知发送给主机。
 * 4. 处理主机对特征的读写请求。
 *
 * 结构体说明：
 * - msg_data_t：串口接收数据结构体，包含数据指针和长度。
 *
 * 全局变量说明：
 * - g_msg_queue：消息队列句柄，用于串口与BLE任务间通信。
 * - g_connection_state：BLE连接状态。
 * - g_uart_rx_buffer：串口接收缓冲区。
 * - g_uart_mtu：BLE传输单元最大长度。
 * - g_uuid_app_uuid：应用UUID。
 * - g_ble_uart_server_addr：BLE服务端MAC地址。
 * - g_notification_characteristic_att_hdl：通知特征句柄。
 * - g_conn_hdl：BLE连接句柄。
 * - g_server_id：BLE服务端ID。
 *
 * 主要函数说明：
 * - stream_data_to_uuid：将uint16类型UUID转换为bt_uuid_t结构体。
 * - compare_service_uuid：比较两个UUID是否相等。
 * - ble_server_add_characters_and_descriptors：添加特征和描述符。
 * - ble_server_service_add_cbk：服务添加回调。
 * - ble_server_characteristic_add_cbk：特征添加回调。
 * - ble_server_descriptor_add_cbk：描述符添加回调。
 * - ble_server_service_start_cbk：服务启动回调。
 * - ble_server_receive_write_req_cbk：写特征值请求回调。
 * - ble_server_receive_read_req_cbk：读特征值请求回调。
 * - ble_server_adv_enable_cbk/ble_server_adv_disable_cbk：广播使能/关闭回调。
 * - ble_server_connect_change_cbk：连接状态变化回调。
 * - ble_server_mtu_changed_cbk：MTU变化回调。
 * - ble_server_enable_cbk：BLE服务端使能回调。
 * - ble_uuid_add_service：添加服务。
 * - ble_server_send_report：通过BLE通知发送数据。
 * - ble_server_register_callbacks：注册GAP和GATT回调。
 * - ble_server_init：BLE服务端初始化。
 * - ble_main_task：BLE主任务，处理消息队列。
 * - ble_server_entry：BLE服务入口，创建任务和消息队列。
 * - ble_uart_server_read_handler：串口接收回调，将数据写入消息队列。
 * - app_uart_init_config：串口初始化及回调注册。
 */
