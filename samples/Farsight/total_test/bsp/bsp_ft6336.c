...
uint32_t FT6336_readByte(uint8_t addr)
{
    ...
    uint32_t result = uapi_i2c_master_write(FT6336_I2C_IDX, I2C_ADDR_FT6336, &i2cData);
    if (result != ERRCODE_SUCC) {
        printf("I2C Write result is 0x%x!!!\r\n", result);
        uapi_i2c_deinit(FT6336_I2C_IDX);
        FT6336_init();
        return result;
    }
...
void gpio_callback_func(pin_t pin, uintptr_t param)
{
    ...
    sprintf_s(str, sizeof(str), "X: %03d,Y: %03d", touchPoint.tp[id1].x, touchPoint.tp[id1].y);
...
FT6336_TouchPointType FT6336_scan(void)
{
    ...
    id1 = FT6336_read_touch1_id(); // id1 = 0 or 1
