 ...
 */
                             ...
                             errcode_t status)
 {
     ...
     // 检查数据长度是否与上报结构体匹配
     if (data->data_len != sizeof(report_data_t)) {
         osal_printk("%s Received data of unexpected size %u from conn_id %u\r\n", SLE_CLIENT_LOG, data->data_len, conn_id);
         return;
     }

     report_data_t *received_data = (report_data_t *)data->data;

     // 打印收到的数据信息
     osal_printk("%s [DATA RECV] from child conn_id %u. Origin MAC: %02x:%02x:**:**:%02x:%02x, Data: %u\r\n",
                 SLE_CLIENT_LOG,
                 conn_id,
                 received_data->origin_mac[0], received_data->origin_mac[1],
                 received_data->origin_mac[4], received_data->origin_mac[5],
                 received_data->data);

     // 如果当前节点是成员(有父节点)，则将数据向上转发
